...
async function Ae(e, t, n = []) {
	for (let r in t) t[r] instanceof W ? await je(e, t, r) : typeof t[r] == "object" && t[r] !== null && await Ae(e, t[r], [...n, r]);
}
	...
	switch (o) {
		...
		case "PAUSED":
			n({
				...
				detail: o,
				discussions_enabled: await Ie(s)
			});
			break;
		case "RUNNING":
...
async function Be(e, t) {
	...
	if (i && !ze.includes(i)) throw Error(`Invalid hardware type provided. Valid types are: ${ze.map((e) => `"${e}"`).join(",")}.`);
	...
	try {
		...
		let i = await r.json();
		return await Re(`${d}/${f}`, a || 300, n), await $.connect(Ve(i.url), t);
	} catch (e) {
		throw Error(e);
	}
}
...
var He = "supports-zerogpu-headers", Ue = !1;
function We() {
...
function Ke() {
	...
	window.addEventListener("message", (e) => {
		e.data === He && (window.supports_zerogpu_headers = !0);
	}), Ue = !0;
	let e = Ge(window.location.hostname);
	e && window.parent !== window && window.parent.postMessage(He, e);
}
//#endregion
...
async function* Ze(e, t) {
	if (!e.body) return;
	let n = Je(e.body), r, i = n.getReader(), a;
	for (;;) {
...
async function Qe(e, t) {
	...
	Xe(n.headers, "Accept", "text/event-stream"), Xe(n.headers, "Content-Type", "application/json");
	let r = await fetch(n);
	if (!r.ok) throw r;
	return Ze(r, n.signal);
}
//#endregion
...
function tt(e, t, n) {
	e[t] ? n.data.forEach((r, i) => {
		let a = nt(i < e[t].length ? e[t][i] : null, r);
		e[t][i] = a, n.data[i] = a;
...
var at = "gradio:run-history:", q = `${at}v2:`, ot = `${at}replay:v2:`, st = 100, ct = 8;
function J(e, t) {
	try {
		return e();
	} catch (e) {
...
function ft(e) {
	let t = Object.keys(window.localStorage).filter((e) => e.startsWith(at)), n = [...t.filter((e) => !e.startsWith(q) && !e.startsWith(ot)), ...t.filter((t) => t.startsWith(q) && t !== e).sort((e, t) => dt(t) - dt(e)).slice(ct - 1)];
	for (let e of n) try {
		window.localStorage.removeItem(e), window.sessionStorage.removeItem(e.replace(q, ot));
	} catch {}
}
function pt() {
...
function gt(e, t) {
	mt(e, Z(e).filter((e) => e.id !== t)), vt();
}
var _t = "gradio:run-history-change";
function vt() {
	if (!(typeof window > "u")) try {
		window.dispatchEvent(new Event(_t));
	} catch {}
}
function yt(e) {
	return typeof window > "u" ? () => {} : (window.addEventListener(_t, e), window.addEventListener("storage", e), () => {
		window.removeEventListener(_t, e), window.removeEventListener("storage", e);
	});
}
function bt(e, t) {
	let n = ut(e);
...
function St(e) {
	...
	return mt(e, [n, ...Z(e)]), vt(), n.id;
}
function Ct(e, t, n) {
	if (!t) return;
	let r = Z(e), i = r.find((e) => e.id === t);
	i && (i.inputs = X(n), mt(e, r));
}
function wt(e, t, n) {
	if (!t) return;
	let r = Z(e), i = r.find((e) => e.id === t);
	...
	else if (n.type === "status" && n.original_msg === "process_starts") Dt(i, n.time);
	else if (n.type === "status" && (n.stage === "generating" || n.stage === "streaming")) i.streamed = !0;
	else if (n.type === "status" && n.stage === "complete") i.status = "completed", Ot(i, n), a = !0;
	else if (n.type === "status" && n.stage === "error") i.status = "failed", i.error = typeof n.message == "string" ? n.message : JSON.stringify(n.message || "Unknown error"), Ot(i, n), a = !0;
	else return;
	if (a) Q.has(t) && (i.outputs = Q.get(t)), Q.delete(t), Et.delete(t);
	else if (i.streamed) {
		if (Date.now() - (Et.get(t) ?? 0) < Tt) return;
		Et.set(t, Date.now()), Q.delete(t);
	}
	mt(e, r);
}
var Tt = 500, Et = /* @__PURE__ */ new Map(), Q = /* @__PURE__ */ new Map();
function Dt(e, t) {
...
function Rt(e, t = {}, n, r, i, a) {
	try {
		...
		let { fn_index: w, endpoint_info: T, dependency: E } = Bt(p, e, m, l), ae = ye(t, T), D = l.protocol ?? "ws";
						...
						let t = async function(e) {
							try {
								...
								console.error("Unexpected client exception", e), R({
									...
									"sse_v2.1",
									"sse_v3"
								].includes(D) && (et(h, C.abort_controller), h.open = !1, V());
							}
						};
...
var $ = class {
	...
	stream(e) {
		...
		return this && this.cookies && t.append("Cookie", this.cookies), this && this.options.headers && new Headers(this.options.headers).forEach((e, n) => {
			t.append(n, e);
		}), this && this.options.token && t.append("Authorization", `Bearer ${this.options.token}`), this.abort_controller = new AbortController(), this.stream_instance = it(e.toString(), {
			credentials: this.options.credentials ?? "same-origin",
			headers: t,
	...
	constructor(e, t = { events: ["data"] }) {
		this.app_reference = e, this.deep_link = t.query_params?.deep_link || null, t.events ||= ["data"], k(t), this.options = t, this.current_payload = {}, t.cookies && (this.cookies = t.cookies), this.view_api = be.bind(this), this.upload_files = z.bind(this), this.handle_blob = Oe.bind(this), this.post_data = Me.bind(this), this.submit = Rt.bind(this), this.predict = Ne.bind(this), this.open_stream = $e.bind(this), this.resolve_config = j.bind(this), this.resolve_cookies = N.bind(this), this.upload = H.bind(this), this.fetch = this.fetch.bind(this), this.handle_space_success = this.handle_space_success.bind(this), this.stream = this.stream.bind(this);
	}
	async init() {
		Ke(), this.options.auth && await this.resolve_cookies(), await this._resolve_config().then((e) => e?.config && this._resolve_heartbeat(e.config));
		try {
	...
	close() {
		this.closed = !0, et(this.stream_status, this.abort_controller);
	}
	async refresh() {
		if (!this.config) throw Error(b);
	...
	async config_success(e) {
		if (this.config = e, this.api_prefix = e.api_prefix || "", e.run_history === !1 && At({
			app_id: e.app_id,
			username: e.username
...
async function Vt(e, t = { events: ["data"] }) {
...
//#endregion
export { $ as Client, U as FileData, ae as MISSING_CREDENTIALS_MSG, At as clear_run_history, Vt as client, Nt as consume_run_history_replay, jt as delete_run_history, Ht as duplicate, De as handle_file, Lt as on_run_history_change, Ne as predict, xe as prepare_files, kt as read_run_history, lt as run_history_url, Mt as stage_run_history_replay, Rt as submit, o as t, H as upload, z as upload_files };
